...
class DistilBertEmbedding(nn.Module):
    def __init__(self, text_agnostic=False):
        ...
        # Load pretrained BERT model if not text_agnostic, else random BERT.
        if text_agnostic:
            self.embedder = AutoModel.from_config(
                AutoConfig.from_pretrained("distilbert-base-uncased")
            )
        else:
            self.embedder = AutoModel.from_pretrained("distilbert-base-uncased")
        # Freeze the model.
        for param in self.embedder.parameters():
            param.requires_grad = False
...
class SplitVisualProjection(nn.Module):
    def __init__(self, visual_dim, split_points, split_dims, dim, drop_rate=0.0):
        super().__init__()
        self.drop = nn.Dropout(p=drop_rate)
        linear_modules = []
        split_points.append(visual_dim)
        start = 0
        feat_segs = []
        if len(split_dims) == 0:
            split_dims = [dim for _ in split_points]
        for sp, sd in zip(split_points, split_dims):
            end = sp
            linear = Conv1D(
                in_dim=end - start,
                out_dim=sd,
            ...
            linear_modules.append(linear)
            feat_segs.append((start, end))
            start = end
        self.feat_segs = feat_segs
        self.linear_modules = nn.ModuleList(linear_modules)
        self.merge_linear = nn.Sequential(
            nn.GELU(),
            Conv1D(
                in_dim=sum(split_dims),
                out_dim=dim,
                kernel_size=1,
                stride=1,
                bias=True,
    ...
    def forward(self, visual_features):
        ...
        visual_features = self.drop(visual_features)
        outputs = []
        for i in range(len(self.feat_segs)):
            s, e = self.feat_segs[i]
            out = self.linear_modules[i](visual_features[:, :, s:e])
            outputs.append(out)
        outputs = torch.cat(outputs, dim=2)
        output = self.merge_linear(outputs)  # (batch_size, seq_len, dim)
        return output
...
class HighLightLayer(nn.Module):
    ...
    @staticmethod
    def compute_distillation_loss(scores_l, scores_e, mask, epsilon=1e-12):
        # (B, L) sigmoids
...
class ConditionedPredictor(nn.Module):
    ...
    def compute_distillation_loss(
        ...
        # all are (B, L) floats
        start_logprob_l = nn.LogSoftmax(dim=1)(start_logits_l)
        end_logprob_l = nn.LogSoftmax(dim=1)(end_logits_l)
        start_prob_e = nn.Softmax(dim=1)(start_logits_e)
        end_prob_e = nn.Softmax(dim=1)(end_logits_e)
        start_loss = nn.KLDivLoss(reduction="batchmean")(start_logprob_l, start_prob_e)
        end_loss = nn.KLDivLoss(reduction="batchmean")(end_logprob_l, end_prob_e)
        return start_loss + end_loss
