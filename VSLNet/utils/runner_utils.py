...
def eval_test(
    ...
    with torch.no_grad():
        for idx, (records, vfeats, vfeat_lens, word_ids, char_ids) in tqdm(
            ...
            _, start_logits, end_logits, sampler_masks = model(
                word_ids, char_ids, vfeats, video_mask, query_mask, niters=niters
            )
            ...
            if sampler_masks is None:
                save_sampler_masks = False
                sampler_masks = [None] * len(records)
            if save_sampler_masks:
                sampler_masks = (
                    sampler_masks.cpu().numpy().astype(np.uint8)
                )  # (B, niters, L) or (B, L)

            # Record output and use standard evalution script for NLQ.
            for record, starts, ends, smask in zip(
                records, start_indices, end_indices, sampler_masks
            ):
                ...
                if save_sampler_masks:
                    new_datum["sampler_mask"] = smask.tolist()
                    new_datum["video_duration"] = record["duration"]
                    new_datum["v_len"] = record["v_len"]
    ...
    if gt_json_path:
        ...
        sampling_efficiency = np.concatenate(sampling_efficiency, axis=0).mean().item()
        display_results = ego4d_eval.display_results(
            results, mIoU, thresholds, topK, title=title
        )
        display_results[1]["Sampling efficiency"] = sampling_efficiency
    else:
        results = None
        mIoU = None
