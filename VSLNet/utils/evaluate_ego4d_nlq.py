...
def display_results(results, mIoU, thresholds, topK, title=None):
    ...
    mIoU *= 100
    values = [
        results[jj][ii] for ii in range(len(topK)) for jj in range(len(thresholds))
    ] + [mIoU]
...
def evaluate_nlq_performance_per_clip_durations(
    predictions, ground_truth, thresholds, topK, clip_durations
):
    """Evalutes the performances."""
    gt_dict = {}
    clip2duration = {}
    ...
    for video_datum in ground_truth["videos"]:
        for clip_datum in video_datum["clips"]:
            clip_uid = clip_datum["clip_uid"]
            clip2duration[clip_uid] = (
                clip_datum["clip_end_sec"] - clip_datum["clip_start_sec"]
            )
            for ann_datum in clip_datum["annotations"]:
                key = (clip_uid, ann_datum["annotation_uid"])
                gt_dict[key] = ann_datum
    ...
    per_duration_results = {
        seg: [[[] for _ in topK] for _ in thresholds] for seg in clip_durations
    }
    per_duration_average_IoU = {seg: [] for seg in clip_durations}
    counts = {seg: 0 for seg in clip_durations}
    clips_per_duration = {seg: set() for seg in clip_durations}
    for pred_datum in predictions:
        key = (pred_datum["clip_uid"], pred_datum["annotation_uid"])
        clip_dur = clip2duration[pred_datum["clip_uid"]]
        clip_dur_key = None
        for seg in clip_durations:
            if clip_dur >= seg[0] and clip_dur < seg[1]:
                assert clip_dur_key is None
                clip_dur_key = seg
        clips_per_duration[clip_dur_key].add(pred_datum["clip_uid"])
        assert key in gt_dict, "Instance not present!"
        query_id = pred_datum["query_idx"]
        gt_datum = gt_dict[key]
        gt_query_datum = gt_datum["language_queries"][query_id]
        ...
        per_duration_average_IoU[clip_dur_key].append(np.mean(np.sort(overlap[0])[-3:]))
        counts[clip_dur_key] += 1
        for tt, threshold in enumerate(thresholds):
            for rr, KK in enumerate(topK):
                per_duration_results[clip_dur_key][tt][rr].append(
                    (overlap > threshold)[:KK].any()
                )
        num_instances += 1

    mean_results = {
        k: np.array(v).mean(axis=-1) for k, v in per_duration_results.items()
    }
    mIoU = {k: np.mean(v) for k, v in per_duration_average_IoU.items()}
    print(f"Evaluated: {num_instances} / {num_gt_queries} instances")
    for k, v in counts.items():
        print(
            "({:6.2f}, {:6.2f}) sec length clips: # queries = {:6.2f}, # clips = {:6d}".format(
                k[0], k[1], v, len(clips_per_duration[k])
            )
        )
    return mean_results, mIoU
