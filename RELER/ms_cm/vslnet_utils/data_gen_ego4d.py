...
def dataset_gen_bert(
    data, vfeat_lens, max_pos_len, scope, num_workers=1, use_cheap_time2index=False
):
    # Worker method for multiprocessing.
    def worker(worker_data, vfeat_lens, max_pos_len, scope, worker_id, output_q):
        ...
        for record in tqdm(worker_data, total=len(worker_data), desc=description):
            ...
            if vid not in vfeat_lens:
                continue
            if use_cheap_time2index:
                s_ind, e_ind, _ = time_to_index_cheap(
                    record["s_time"],
                    record["e_time"],
                    vfeat_lens[vid],
                    record["duration"],
...
def gen_or_load_dataset(configs):
    ...
    feature_dir = os.path.join(
        configs.vslnet_datapath, "features", configs.task, configs.fv
    )
