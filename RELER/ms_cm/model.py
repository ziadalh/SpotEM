...
from ms_cm.transformer import build_cross_self_encoder_fixcross
from ms_cm.vslnet_model.layers import NPM, ConditionedPredictor
...
class MS_CM(nn.Module):
    ...
    def __init__(
        ...
        if args.use_feature_sampler:
            args.cross_hidden_dim = hidden_dim
            if args.feature_sampler_type == "random":
                sampler_cls = RandomSampler
            elif args.feature_sampler_type == "uniform":
                sampler_cls = UniformSampler
            elif args.feature_sampler_type == "zero":
                sampler_cls = ZeroSampler
                assert args.feature_sampler_efficiency == 1.0
            elif args.feature_sampler_type == "all":
                sampler_cls = AllSampler
                assert args.feature_sampler_efficiency == 0.0
            elif args.feature_sampler_type == "transformer-v1":
                sampler_cls = TransformerV1Sampler
            elif args.feature_sampler_type == "liteeval":
                sampler_cls = LiteEvalSampler
            elif args.feature_sampler_type == "ocsampler":
                sampler_cls = OCSampler

            self.sampler = sampler_cls.from_configs(args)
    ...
    def forward(
        ...
        sampler_masks = None
        if self.sampler:
            src_vid_total, sampler_masks = self.sampler(
                src_vid_total,
                src_txt=src_txt,
                src_vid_mask_total=src_vid_mask_total,
                src_txt_mask=src_txt_mask,
                pos_txt=pos_txt,
                input_vid_proj=self.input_vid_proj,
                position_embed=self.position_embed,
                cross_self_encoder=self.cross_self_encoder,
                gs_tau=gs_tau,
                niters=niters,
                cache=self._sampler_cache,
            )
            if cache_samples:
                self.populate_sampler_cache({"sampler_masks": sampler_masks})
        ...
        vid_mem_total = torch.cat(vid_mem_total_list, dim=1)
        ...
        out["video_memory"] = vid_mem_total  # (bsz, L_vid, d)
        ...
        out["sampler_masks"] = sampler_masks

        return out

    def populate_sampler_cache(self, x):
        self._sampler_cache = x

    def delete_sampler_cache(self):
        self._sampler_cache = None

    def compute_highlight_distillation_loss(
        self, scores_l, scores_e, mask, epsilon=1e-12
    ):
        # (B, L) sigmoids
        loss_per_location = nn.BCELoss(reduction="none")(scores_l, scores_e)
        mask = mask.type(torch.float32)
    ...
    def compute_npm_distillation_loss(self, scores_l, scores_e, mask):
        return self.NPM_proj.compute_distillation_loss(scores_l, scores_e, mask)

    def compute_saliency_distillation_loss(self, scores_l, scores_e):
        # scores are per-time-step logits (B, L)
        probs_l = nn.Sigmoid()(scores_l)
        probs_e = nn.Sigmoid()(scores_e)
        return nn.BCELoss()(probs_l, probs_e)
...
def build_model(args):
    ...
    if args.video_frame_contrastive_loss:
        weight_dict["loss_video_frame_contrastive"] = (
