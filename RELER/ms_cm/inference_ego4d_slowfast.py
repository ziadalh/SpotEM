...
def compute_mr_results(
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        ...
        for scale_tmp in numscale_list:
            ...
            # Cache feature sampling to avoid per-scale sampling
            outputs = model(**model_inputs, niters=niters, cache_samples=True)
        ...
        # Update sampling efficiencies
        if model.sampler:
            srate = model.sampler.compute_sampling_rate(
                outputs["sampler_masks"], outputs["video_mask"]
            )
        else:
            srate = torch.ones_like(outputs["video_mask"].shape[0])
        seff = (1.0 - srate).cpu().numpy()
        sampling_efficiency.append(seff)
    ...
    if gt_json_path:
        ...
        if return_results_dict:
            ...
            sampling_efficiency = np.concatenate(sampling_efficiency, axis=0)
            results_dict["Sampling efficiency"] = sampling_efficiency.mean().item()
        else:
            results_dict = None
...
def setup_model(opt, is_distributed=False, local_rank=0, world_rank=0):
    ...
    if opt.optim_name == "AdamW":
        if opt.use_feature_sampler:
            start_key = "module.sampler." if is_distributed else "sampler."
            sampler_params = []
            task_params = []
            for n, p in model.named_parameters():
                if not p.requires_grad:
                    continue
                if n.startswith(start_key):
                    sampler_params.append(p)
                else:
                    task_params.append(p)
            param_dicts = [
                {"params": sampler_params, "lr": opt.lr * opt.lr_scale_sampler},
                {"params": task_params},
            ]
        else:
            param_dicts = [
...
def start_inference():
    ...
    opt.vsldataset_task = "nlq_aug_nar"
    ...
    train_gt_json = "../episodic-memory/NLQ/VSLNet/data/nlq_train.json"
    opt.v_feat_dirs = [
        "../episodic-memory/NLQ/VSLNet/data/features/nlq_official_v1/egovlp",
    ...
    with torch.no_grad():
        results, mIoU, score_str = eval_epoch(
