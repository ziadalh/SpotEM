...
from utils.basic_utils import dict_to_markdown, load_json, save_json
...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--lr_scale_sampler", type=float, default=1.0)
        parser.add_argument(
            "--lr_drop",
            type=int,
            default=400,
        ...
        parser.add_argument(
            ...
            default=1,
            type=int,
            help="Evaluation interval after K epochs",
        )
        ...
        parser.add_argument(
            "--feature_sampler_efficiency_offset",
            default=0.0,
            type=float,
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--use_sampler_cumulative_loss",
            action="store_true",
            default=False,
            help="use cumulative loss instead of per-step loss for sampler",
        )
        parser.add_argument(
        ...
        parser.add_argument("--benchmark_model_path", type=str, default="")
    ...
    def parse(self):
        ...
        if isinstance(self, TestOptions):
            if opt.use_feature_sampler:
                opt.model_dir = "/".join(opt.resume.split("/")[:-2])
            else:
            ...
            for (
                ...
                if arg not in ["results_root", "num_workers", "resume", "resume_all"]:
