...
class Ego4d_dataset(Dataset):
    ...
    def __init__(
        ...
        if self.mode == "eval" or self.mode == "test":
            ...
            assert nar_rand_translate is False
    ...
    def __getitem__(self, index):
        ...
        model_inputs["query_feat"] = self._get_query_feat_by_qid(
            record["annotation_uid"], record["query_idx"]
        )  # (Dq, ) or (Lq, Dq)
        ...
        if self.use_video:
            (
                ...
                s_ind,
                e_ind,
            ) = self._get_video_feat_by_vid(record, record_random)  # (Lv, Dv)
    ...
    def _get_query_feat_by_qid(self, annotation_uid, query_idx):
        q_feature = torch.load(
            os.path.join(self.q_feat_dir, "{}_{}.pt".format(annotation_uid, query_idx))
        )
    ...
    def _get_video_feat_by_vid(self, record, record_random):
        ...
        for idx, _feat_dir in enumerate(self.v_feat_dirs):
            ...
            if self.use_sw and self.mode == "train":
                if idx == 0:
                    ...
                    s_ind, e_ind = (
                        math.floor(
                            min(gt_s_time / record["duration"], 1.0)
                            * (total_feat_len - 1)
                        ),
                        math.ceil(
                            min(gt_e_time / record["duration"], 1.0)
                            * (total_feat_len - 1)
                        ),
                    )
        ...
        if self.use_sw and self.mode == "train":
            ...
            s_ind = max(0, int(record["s_ind"]))
            e_ind = min(vlen - 1, int(record["e_ind"]))
...
def Ego4d_collate(batch):
    ...
    batch_s_ind = [max(0, int(record["s_ind"])) for record in batch_record]
    batch_e_ind = [
        min(int(record["e_ind"]), record["v_len"] - 1) for record in batch_record
    ]
