...
ctx_mode=video_tef
v_feat_types=egovlp_clip_object_room_interaction
t_feat_type=clip
results_root=$EXPT_ROOT/efficiency_${EFFICIENCY}_slc_${LOSS_COEF}_lr_${LR}_${LR_SCALE}_dcoefs_${D_L}_${D_F}_${D_H}_${D_S}_${D_N}
...
PYTHONPATH=$PYTHONPATH:. python -W ignore -u -m torch.distributed.launch \
    ...
    --master_port $MASTER_PORT \
     ms_cm/train_ego4d_slowfast.py \
        ...
        --sampler_niters $NITERS \
        --resume ../pretrained_models/reler/uniform_rio_init.t7 \
        --lr $LR \
        --lr_scale_sampler $LR_SCALE \
        --n_epoch $EPOCHS \
        --use_full_distillation \
        --distillation_ckpt_path ../pretrained_models/reler/spotem_distill_teacher.t7 \
        --distill_l_coef $D_L \
        --distill_f_coef $D_F \
        --distill_h_coef $D_H \
        --distill_s_coef $D_S \
